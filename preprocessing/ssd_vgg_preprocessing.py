...
#模型测试函数
def preprocess_for_eval(image,           #输入图片
                                labels,  #标签
                                bboxes,  #边框（groundtruth bboxes）
                                out_shape=EVAL_SIZE, #输出形状(300*300) 
                                data_format='NHWC',  #输入数据格式[batch,height,width,channel]
                                difficults=None,     #
                                resize=Resize.WARP_RESIZE,  #图片形状操作（crop/wrap）
                                scope='ssd_preprocessing_train'):#节点命名空间
    ...
    with tf.name_scope(scope):
        if image.get_shape().ndims != 3:   #获得输入图片维度的个数
            raise ValueError('Input must be of size [height, width, C>0]')

        image = tf.to_float(image)
        image = tf_image_whitened(image, [_R_MEAN, _G_MEAN, _B_MEAN])  #白化，去均值
        ...
        if resize == Resize.NONE:
            # No resizing...
            pass
        elif resize == Resize.CENTRAL_CROP: #裁剪操作
            # Central cropping of the image.
            # 从中心向外把图像裁剪为300*300大小
        ...
        if difficults is not None:
            mask = tf.logical_not(tf.cast(difficults, tf.bool)) #返回x中每个素的逻辑非。只适用于dtype为tf.bool的Tensor对象
            labels = tf.boolean_mask(labels, mask) #将使a 矩阵仅保留与b中“True”元素同下标的部分
            bboxes = tf.boolean_mask(bboxes, mask)
        # Image data format.
        if data_format == 'NCHW': #[batch,channel,height,width]
            image = tf.transpose(image, perm=(2, 0, 1))  #交换输入张量的不同维度[h,w,c]->[c,h,w]
        return image, labels, bboxes, bbox_img  #[输入图片，标签，resize图片后groudtruth bbox，resize图片后相对于原图的坐标框]
